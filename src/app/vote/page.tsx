  ...
  const fetchData = useCallback(async () => {
    try {
      const [meRes, statusRes] = await Promise.all([
        fetch("/api/auth/me"),
        fetch(`/api/vote/status?electionId=${process.env.NEXT_PUBLIC_ELECTION_ID || ""}`),
      ]);
      ...
      if (statusData.election?.id) {
        const candRes = await fetch(
          `/api/candidates/${statusData.election.id}`,
        );
        const candData = await candRes.json();
        setPositions(candData.positions || []);
        setCandidates(candData.candidates || []);

        // Load results if applicable
        if (statusData.hasVoted && statusData.election.showLiveResults) {
          const resRes = await fetch(
            `/api/results/${statusData.election.id}`,
          );
          if (resRes.ok) {
